...
const char cmd1[] = {"5001AA"}; // Comando para encender led verde
const char cmd2[] = {"5000AA"}; // Comando para apagar led verde
const char cmd3[] = {"5101AA"}; // Comando para encender led rojo
const char cmd4[] = {"5100AA"}; // Comando para apagar led rojo
const char cmd5[] = {"5201AA"}; // Comando para iniciar impresión de contador
const char cmd6[] = {"5200AA"}; // Comando para parar impresion de contador
...
int main(void) {
    ...
    while (1) {
        ...
        {
            if (U1_PrintRate_ISR++ >= 50) // Determinar el ritmo de transmision de datos de la Uart1 Tx. Por ejemplo, 10 veces el valor de delay del bucle.(delay_ms(10))
            {
                if (contador <= 100) {
                    memset(txbuffer_ISR, '\0', sizeof (txbuffer_ISR)); // Clear Buffer and fill it with NULL
                    sprintf(txbuffer_ISR, "IMPRIMIENDO DATOS: %d %s\r\n", contador++);

                    nextchar = 0;
                    BufferLoadDone = 0;
                    U1_PrintRate_ISR = 0;
                    
                    if (contador == 100) contador = 0; //Reseteo del contador si llega a 100
